...
def cyclic_learning_rate(global_step,
    ...
    with ops.name_scope(name, "CyclicLearningRate",
        ...
        if global_step.dtype != tf.float32:
            global_step = tf.cast(global_step, tf.float32)
        step_size = math_ops.cast(step_size, dtype)
        if step_size.dtype != tf.float32:
            step_size = tf.cast(step_size, tf.float32)
        if learning_rate.dtype != tf.float32:
        ...
        def cyclic_lr():
            ...
            double_step = tf.multiply(2.00000, step_size)
