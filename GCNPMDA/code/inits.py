...
def load_data(train_arr, test_arr):
    ...
    #csr_matrix表示逐行(注意csr的r,row)压缩矩阵
    logits_test = sp.csr_matrix((labels[test_arr,2],(labels[test_arr,0]-1, labels[test_arr,1]-1)),shape=(nd,nm)).toarray()
    logits_test = logits_test.reshape([-1,1])#（（292*39），1） 0-1之间的数

    logits_train = sp.csr_matrix((labels[train_arr,2],(labels[train_arr,0]-1, labels[train_arr,1]-1)),shape=(nd,nm)).toarray()
    logits_train = logits_train.reshape([-1,1])#（（292*39），1） 0-1之间的数

    train_mask = np.array(logits_train[:,0], dtype=np.bool).reshape([-1,1])#（（292*39），1） false
    test_mask = np.array(logits_test[:,0], dtype=np.bool).reshape([-1,1])#（（292*39），1） false

    M = sio.loadmat('data/'+dataset+'/interaction.mat')
    ...
    F1 = np.loadtxt("data/"+dataset+"/disease_features.txt")#39*39
    F2 = np.loadtxt("data/"+dataset+"/microbe_features.txt")#292*292
    ...
    features = normalize_features(features)#（292+39）*（292+39）
