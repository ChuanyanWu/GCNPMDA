...
class GraphConvolution():
    ...
    def __init__(self, input_dim, output_dim, adj, name, dropout=0., act=tf.nn.leaky_relu):#tf.nn.relu
        self.name = name
        self.vars = {}
        self.issparse = False
...
class GraphConvolutionSparse():
    ...
    def __init__(self, input_dim, output_dim, adj, features_nonzero, name, dropout=0.,act=tf.nn.leaky_relu):#tf.nn.relu
        self.name = name
        self.vars = {}
        with tf.variable_scope(self.name + '_vars'):
            self.vars['weights'] = weight_variable_glorot(
